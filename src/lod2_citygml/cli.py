...
def build(
    ...
    relative_height: Optional[Path] = typer.Option(None, "--relative-height", help="DSM-DTM difference raster (single-band TIFF, VRT, or ECW)."),
    ...
    citygml_version: Optional[str] = typer.Option(None, "--citygml-version", help="2.0 or 3.0."),
    ...
    max_buildings: Optional[int] = typer.Option(None, "--max-buildings", help="Large-area threshold."),
    max_area_km2: Optional[float] = typer.Option(None, "--max-area-km2", help="Large-area threshold in km²."),
    on_large_area: Optional[str] = typer.Option(None, "--on-large-area", help="warn | abort | tile"),
    min_roof_confidence: Optional[float] = typer.Option(None, "--min-roof-confidence", help="0-1 threshold."),
    roof_shape: Optional[str] = typer.Option(None, "--roof-shape", help="mbr | footprint"),
    lod1_intermediate: Optional[Path] = typer.Option(
        None,
        "--lod1-intermediate",
        help="Optional output for intermediate LOD1 footprints (GPKG).",
    ),
    tile_size: Optional[int] = typer.Option(None, "--tile-size", help="Tile width for chunking mode."),
